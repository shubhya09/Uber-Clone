...
export const createRide = asyncHandler(async (req, res) => {
  ...
  const fareDetails = await calcFare(pickUp, drop);
  const { fares, distanceInKm, durationInMins } = fareDetails;
  const otp = generateOtp(6);
...
export const getFare = asyncHandler(async (req, res) => {
  const { pickup, drop } = req.body;
  if (!pickup || !drop) {
    return res.status(400).json({ message: "Missing pickup or drop" });
  }
...
export const cancelRide = asyncHandler(async (req, res) => {
  ...
  const { status, fare } = ride;

  if (status === "pending") {
    await RideModel.findByIdAndUpdate(rideId, { status: "cancelled" });
    return res.status(200).json({ message: "Pending ride cancelled successfully" });
  }
...
export { getNearbyCaptains as getNearbyPolice } from "../utils/RideServices.js";
...
export const seeRides = asyncHandler(async (req, res) => {
  const rides = await RideModel.find({ User: req.user._id }).populate("Captain");
